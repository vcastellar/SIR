...
#' SIRS model with waning immunity, cumulative infections and incidence output
#' @name sirs
#' @description
#' Defines the right-hand side (derivatives) of an SIRS compartmental model
#' (S \eqn{\rightarrow} I \eqn{\rightarrow} R \eqn{\rightarrow} S) extended with an
#' auxiliary cumulative-infections state \code{C}. This function is intended to be
#' passed as the \code{func} argument to \code{deSolve::ode()}.
#'
#' @details
#' ## State variables
#' The state vector \code{state} must contain (at least) the following named
...
#'   \item{beta}{Transmission rate (per day).}
#'   \item{gamma}{Recovery/removal rate (per day).}
#'   \item{omega}{Waning-immunity rate (per day), moving individuals from \code{R} back to \code{S}.}
#' }
#'
#' ## Model equations
#' The total population is computed as \eqn{N = S + I + R}. The incidence
...
#'   \code{S}, \code{I}, \code{R}, and \code{C}.
#' @param parms Named numeric vector of model parameters. Must include
#'   \code{beta}, \code{gamma}, and \code{omega}.
#'
#' @return A list with two elements:
#' \describe{
#'   \item{1}{A numeric vector of derivatives \code{c(dS, dI, dR, dC)} (in this order).}
...
#' times <- 0:160
#'
#' out <- deSolve::ode(y = init, times = times, func = sirs, parms = parms, method = "lsoda")
#' head(out)
#'
#' # Plot infectious prevalence I(t)
#' plot(out[, "time"], out[, "I"], type = "l", xlab = "Days", ylab = "I(t)")
